...
parse_remote_state() {
	...
	if [[ "$remote_state" != "" ]]; then
		...
		if [[ "$remote_state" == *ahead* ]] && [[ "$remote_state" == *behind* ]]; then
			behind_num=$(echo "$remote_state" | perl -nle 'print $& if m{behind \d*}' | perl -nle 'print $& if m{\d*$}')
			ahead_num=$(echo "$remote_state" | perl -nle 'print $& if m{ahead \d*}' | perl -nle 'print $& if m{\d*$}')
